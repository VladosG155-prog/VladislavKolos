	...
	render() {
		return (
			<div className="header__last">
				<Currencies />
				{this.state.isOpenCart && <div className="overlay"></div>}
				<div
					onClick={() => this.setState({ isOpenCart: true })}
					...
					{this.state.isOpenCart && (
						<div className="mobile-cart">
							<p>
								<b>My Bag,</b> {this.props.cartCount} items
							</p>
							{this.props.items.map((item) => (
								<CartItem key={`${item.id}-menu`} {...item} />
							))}
							<div className="total-mincart-price">
								<p>Total:</p>
								<p>
							...
							<div className="cart__buttons">
								<Link to="/cart" className="cart-btn-first btn">
									View Bag
								</Link>
					...
					{this.props.cartCount > 0 ? (
						<span className="countInCart">{this.props.cartCount}</span>
					) : null}
				</div>
			</div>
		);
...
export default connect(mapStateToProps)(HeaderLast);
